...
sys_open(void)
{
  ...
  if(omode & O_CREATE){
    ...
    //symbolic link with follow
    if (ip->type == T_SYMLINK && (omode & O_NOFOLLOW) == 0){
      int depth = 0;
      while (ip->type == T_SYMLINK){
        int readbytes = 0;
        readi(ip, 0, (uint64) (&readbytes), 0, sizeof(int));
        readi(ip, 0, (uint64) path, sizeof(int), readbytes + 1);
...
sys_symlink(void)
{
  ...
  writei(ip, 0, (uint64) (&targetlen), 0, sizeof(int));
  writei(ip, 0, (uint64) target, sizeof(int), targetlen + 1);
  iunlockput(ip);
  end_op();
